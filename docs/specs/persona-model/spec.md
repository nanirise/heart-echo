...
> ⚠️ **本功能遵循队长于 2026-09-13 明确的「通用错误码规则」**：**资源越权 → `4043`**（隐藏资源存在性）、**功能越权 → `4030`**。人设 CRUD 属于前者，故 `:id` 未命中一律 `4043`，见 §5.2。
> 契约 §4 目前仍写着 `4030`/`4043`，**规则广播与契约同步完成前不合并 PR**；全局文件由队长/成员 1 统一处理，本分支只改这两份文档。
...
| 新增错误码 | 本功能**一个都不需要新增**，复用 `4001` / `4043` / `5003` |
...
> **不要**给列表加"查不到就 4043"的逻辑：**没有**任何人设是**正常状态**（总纲 §0.3 明确要求前端做空状态引导），返回错误码会让前端无法区分"我还没有人设"和"请求失败"。
...
3. **幂等性**：删第二次返回 `4043`（与"不存在"同一个码，天然幂等且不泄漏）。前端二次确认后调用；如果前端重复提交，第二次是 4043 而不是 200——按契约实现即可，不要为它特判。
...
### 5.2 错误码：资源越权 → `4043`（队长规则，2026-09-13）

> **团队通用规则**（队长 2026-09-13 明确）
>
> | 越权类型 | 定义 | 错误码 |
> |---|---|---|
> | **资源越权** | 功能你能用，但这个**资源不属于你**（如用户 A 修改用户 B 的 persona） | **`4043`** —— 隐藏资源存在性 |
...
**本模块没有功能越权场景**：契约 §4 的 4 个端点对所有登录用户一视同仁（本项目无管理员接口、无角色区分），所以**人设模块不会产生 `4030`**。将来若出现真正的功能级权限区分，`4030` 才派上用场。

**代价（诚实记录）**：调试时"改别人的人设"与"改不存在的 id"返回同一个码，排查略麻烦——靠服务端日志区分（`BizErrorHandler` 会为 4xxx 记 warn 日志）。

**实现上反而更简单（附带收益）**：不需要"先带归属条件查、未命中再按 id 探针二分"的两步判定——两步判定的**唯一目的**就是区分这两个码。现在一条 `WHERE id = ? AND user_id = ?` 就够，未命中直接 `4043`，正常路径与异常路径都是**一次查询**。仓储层因此不需要 `ExistsByID`（§5.1 ②）。

**⚠️ 契约尚未写入这条规则 —— 走完流程再合 PR：**

| # | 动作 | 具体位置 |
|---|---|---|
| 1 | **群里广播规则本身**（不只是广播"我改了"） | 成员 2 的 Mock **直接受影响**（他按 `4030`/`4043` 写的错误分支要改） |
| 2 | 更新 `API_CONTRACT.md` §4 端点表 | `PUT /personas/:id` 错误码 `4001 4030 4043` → `4001 4043`；`DELETE /personas/:id` 的 `4030 4043` → `4043` |
| 3 | 更新 `API_CONTRACT.md` §4 末尾说明 | line 185「否则返回 `4030`」改写为「否则返回 `4043`（资源越权，不区分不存在与不属于当前用户，避免泄漏存在性）」 |
| 4 | 登记 `API_CONTRACT.md` §12 变更记录 + 升版本号 | v1 → v2（契约纪律：任何变更必须登记并广播） |
| 5 | **更新 `AGENTS.md` §4.3** | 该节现有「涉及 `:id` 的人设操作必须校验归属，失败返回 `4030`」——**不改这里，后续 AI 编码代理会照着它把 `4030` 加回来** |
| 6 | 更新前端 `types/errcode.ts` | 若成员 2 已把 `4030` 写成"人设越权"分支，需要同步 |
| 7 | **顺带解决 `4030` 的文案** | 契约 §2 里 `4030` 的 msg 是「该人设不属于当前用户」——那是**资源越权**的说法，与它现在承担的语义（功能越权）冲突，需一并改写 |
...
|---|---|---|---|---|
| 1 | `GET /chat/personas/:personaId/messages`（:216） | `4030 4043` | `4043` | 换码 |
| 2 | `POST /chat/stream`（:217，SSE） | `4001 4010 4030 4043 5001 5002` | `4001 4010 4043 5001 5002` | 换码 |
| 3 | `GET /memory`（:281） | `4030 4043` | `4043` | 换码 |
| 4 | `GET /profile/portrait`（:282） | `4030 4043` | `4043` | 换码 |
| 5 | `GET /proactive/settings`（:358） | `4030 4043` | `4043` | 换码 |
| 6 | `PUT /proactive/settings`（:359） | `4001 4030 4043` | `4001 4043` | 换码 |
| 7 | `POST /proactive/trigger`（:360） | `4030 4043 5001` | `4043 5001` | 换码 |
| 8 | `GET /schedules`（:402） | `4001 4030 4043` | `4001 4043` | 换码 |
| 9 | `DELETE /schedules/:id`（:403） | `4030 4043` | `4043` | **只删 `4030`** |
| 10 | `POST /schedules/:id/trigger`（:404） | `4030 4043 5001` | `4043 5001` | **只删 `4030`** |

> 第 1-8 条是"`4030`/`4043` 换 `4043`"；第 9、10 条（日程的两个 `:id` 端点）**已经带 `4043` 了**，只是多带了一个 `4030`——**只删那一个码**即可。这两条恰好说明「一个端点里 `4030` 与 `4043` 并存」是当前契约的既有形态，也正是新规则要清理的。
>
...
| 未命中时返回 `4030` 区分"别人的" | 违反资源越权规则；且泄漏 id 存在性，可被逐号枚举 | 统一 `4043`（§5.2） |
| 用 `4043` 表示"不存在"、`4043` 表示"不属于你" | **两者并存就能被二分探测出 id 是否存在**，等于没隐藏 | 都收敛到 `4043`（§5.2） |
...
| 归属校验必须在 service 层，`4043` 由 service 返回 | 成员 3 任务书 §2 |
...
| `pkg/errcode`（4001/4043/5003 常量） | 成员 1 | **仓库中尚不存在** | service 无法编译 |
...
| ⚠️ **错误码规则：资源越权 → `4043`** | **全员**（成员 2 的 Mock；成员 1 的 memory/chat；成员 3 的 proactive/schedules） | **必须广播的是规则本身，不只是"我改了人设"**：人设端点先落地；契约里**另 10 个**带归属校验的端点按同一规则也应为 `4043`（逐行清单见 §5.2 末尾），但契约与代码由队长统一安排——**不在本 PR 内顺手改** |
...
| 4 | `:id` 未命中统一返回 `4043`（不区分不属于你 / 不存在） | §5.2 |
| 5 | **错误码按队长通用规则**：**资源越权 → `4043`**（隐藏资源存在性）、**功能越权 → `4030`**。人设 CRUD 属资源越权，故 `:id` 未命中一律 `4043`，且**不产生 `4030`** | §5.2 |
...
| 4 | **通用规则的落地范围**：除人设外，那 10 个端点何时统一为 `4043`？ | 已确认**不在本分支改**（用户去群里广播，`API_CONTRACT.md` / `AGENTS.md` 是全局文件）。建议队长排一次"错误码统一"小任务，把 §4 + `§5/§7/§9/§10` 那 10 行 + `AGENTS.md` §4.3 + `4030` 文案一起改干净（清单连带行号见 §5.2 末尾）；本 PR 只负责 §4 人设端点 |
...
- [ ] **资源越权（`:id`）**：用 B 的 Token 打 A 的 `personaId` → **`4043`**（`PUT` 与 `DELETE` 各验一次），且响应 `message` 与下一条**逐字相同**
- [ ] **不存在**：不存在的 id → **`4043`**（与上一条**同码同文案**，这正是 §5.2 隐藏存在性要的效果）
- [ ] **不产生 `4030`**：4 个端点在任意输入组合下（越权 / 不存在 / 缺参 / 无 Token / 超长字段）**都不出现 `4030`**，也不出现 `4043`——本模块没有功能越权场景（§5.2）
...
- [ ] **规则已广播**，且 `API_CONTRACT.md`（§4 + §12 + 版本号 + §2 的 `4030` 文案）与 `AGENTS.md` §4.3 已由队长 / 成员 1 同步更新（清单见 §5.2 表、§7.3 #4）。**本分支不直接改这两个全局文件**
...
| 2026-09-13 | v2 | 并入 4 项决策：① `PageResult[T]` → `internal/dto/common_dto.go`；② ID 统一 `uint64`；③ `POST /personas` 同事务播种 `proactive_settings`（新增 §4.3.1、新增 `model/proactive_setting.go`、`dto/common_dto.go`）；④ **`:id` 未命中统一 `4043`**，取代契约的 `4030`/`4043`（§5.2 改写，含广播与文档更新清单） | 评审决策 |
| 2026-09-13 | v3 | **套用队长「通用错误码规则」**：资源越权 → `4043`（隐藏存在性）、功能越权 → `4030`。§5.2 由「契约偏离」重写为「规则的应用」，补两条推论：①「不存在」也不能用 `4043`（与 `4043` 并存即可被二分探测）；②本模块无功能越权场景，**不产生 `4030`**。§7.2 交接项改为「广播规则本身 + 10 个端点待队长统一」（清单已按契约行号逐条核对并附于 §5.2），§8 新增「不产生 `4030`」验收项。**全局文件（`API_CONTRACT.md` / `AGENTS.md`）不在本分支改动** | 队长裁决 |
