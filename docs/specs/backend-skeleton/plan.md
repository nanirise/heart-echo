...
| 待开 | Step 4–5 | `pkg/logger` + `pkg/jwt` | 代码已完成 |
| 待开 | Step 6–7 | `internal/config` + 中间件 ×5 | 未开始 |
| 待开 | Step 8–10 | 路由装配 + `.env.example` + 冒烟验收 | 未开始 |

**切分判据**：后续步骤只会「**使用**」而不会「**改动**」的部分，就可以先合。

- 可以提前合 —— `pkg/logger` / `pkg/jwt`：Step 6–10 全都只是消费它们，已经定型
